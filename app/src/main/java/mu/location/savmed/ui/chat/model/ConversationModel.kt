...
class ConversationModel @WorkerThread constructor(val chatRoom: ChatRoom) {
    ...
    private val chatRoomListener = object : ChatRoomListenerStub() {
        ...
        override fun onStateChanged(chatRoom: ChatRoom, newState: ChatRoom.State?) {
            org.linphone.core.tools.Log.i("$TAG Conversation state changed [${chatRoom.state}]")
            if (chatRoom.state == ChatRoom.State.Created) {
                subject.postValue(chatRoom.subject)
                computeParticipants()
            } else if (chatRoom.state == ChatRoom.State.Deleted) {
                org.linphone.core.tools.Log.i("$TAG Conversation [$id] has been deleted")
        ...
        override fun onConferenceJoined(chatRoom: ChatRoom, eventLog: EventLog) {
            ...
            org.linphone.core.tools.Log.i("$TAG Conversation has been joined")
            subject.postValue(chatRoom.subject)
            computeParticipants()
        }
        ...
        override fun onConferenceLeft(chatRoom: ChatRoom, eventLog: EventLog) {
            org.linphone.core.tools.Log.w("TAG Conversation has been left")
        ...
        override fun onSubjectChanged(chatRoom: ChatRoom, eventLog: EventLog) {
            org.linphone.core.tools.Log.i("$TAG Conversation subject changed [${chatRoom.subject}]")
            subject.postValue(chatRoom.subject)
        }
        ...
        override fun onEphemeralMessageDeleted(chatRoom: ChatRoom, eventLog: EventLog) {
            org.linphone.core.tools.Log.i("$TAG An ephemeral message lifetime has expired, updating last displayed message")
            updateLastMessage()
        }
    }
    ...
    fun markAsRead() {
        coreContext.postOnCoreThread {
            chatRoom.markAsRead()
            unreadMessageCount.postValue(chatRoom.unreadMessagesCount)
            org.linphone.core.tools.Log.i("$TAG Conversation [$id] has been marked as read")
        }
    }
    ...
    fun toggleMute() {
        coreContext.postOnCoreThread {
            ...
            if (muted) {
                org.linphone.core.tools.Log.i("$TAG Conversation [$id] is now muted")
            } else {
                org.linphone.core.tools.Log.i("$TAG Conversation [$id] is no longer muted")
            }
    ...
    fun call() {
        coreContext.postOnCoreThread {
            ...
            org.linphone.core.tools.Log.i("$TAG Calling [${address.asStringUriOnly()}]")
            address.username?.let { it1 -> coreContext.startCall(it1) }
        }
    }
    ...
    fun delete() {
        coreContext.postOnCoreThread { core ->
            org.linphone.core.tools.Log.i("$TAG Deleting conversation [$id]")
    ...
    fun leaveGroup() {
        coreContext.postOnCoreThread {
            chatRoom.leave()
            org.linphone.core.tools.Log.i("$TAG Group conversation [$id] has been leaved")
            isReadOnly.postValue(true)
        }
    }
    ...
    private fun updateLastMessageStatus(message: ChatMessage) {
        ...
        lastMessageText.postValue(SavMedUtils.getFormattedTextDescribingMessage(message))

        isLastMessageOutgoing.postValue(isOutgoing)
        if (isOutgoing) {
            lastMessageIcon.postValue(SavMedUtils.getChatIconResId(message.state))
        }
    ...
    fun updateLastMessage() {
        ...
        val sender = coreContext.core.findFriend(chatRoom.peerAddress)
        if (sender != null) {
            if (sender.vcard?.givenName != null && sender.vcard?.familyName != null) {
                lastMessageTextSender.postValue("${sender.vcard?.givenName} ${sender.vcard?.familyName}")
            } else {
                lastMessageTextSender.postValue("${sender.name}")
            }
        } else {
            lastMessageTextSender.postValue(chatRoom.peerAddress.username)
        }

        Log.i(TAG,"I am chatRoom user ${chatRoom.peerAddress.username}")
        ...
        if (message != null) {
            ...
            org.linphone.core.tools.Log.w("$TAG No last message to display for conversation [$id]")
        }
    }
    ...
    fun computeParticipants() {
        ...
        val address = if (chatRoom.hasCapability(Capabilities.Basic.toInt())) {
            org.linphone.core.tools.Log.d("$TAG Conversation [$id] is 'Basic'")
            ...
            if (isGroup) {
                ...
                for (participant in chatRoom.participants) {
                    val friend = coreContext.core.findFriend(
                        participant.address
                    )
